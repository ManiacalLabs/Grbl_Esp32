...
void deploy_z(float amt){
	#ifdef SERVO_Z_PIN
		Z_Servo_Axis.deploy(amt);
	#endif
}
...
void ServoAxis::deploy(float amt)
{
	// grbl_send(CLIENT_SERIAL, "[MSG:Deploy Servo: ");
	// grbl_send(CLIENT_SERIAL, String(amt).c_str());
	// grbl_send(CLIENT_SERIAL, "]\r\n");
	_write_pwm((uint32_t)mapConstrain(amt, 0.0, 1.0, SERVO_MIN_PULSE, SERVO_MAX_PULSE ));
}
